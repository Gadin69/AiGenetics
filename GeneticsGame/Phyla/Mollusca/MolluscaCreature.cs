...
/// <summary>
/// Soft-bodied creature implementation
/// Represents creatures with flexible bodies and no rigid skeleton
/// </summary>
public class MolluscaCreature
{
    /// <summary>
    ...
    /// <summary>
    /// Constructor for MolluscaCreature
    /// </summary>
    /// <param name="id">Unique identifier</param>
    /// <param name="genome">Genetic blueprint</param>
    public MolluscaCreature(string id, Genome genome)
    {
        Id = id;
        Genome = genome;
    ...
    public void Update(double timeStep)
    {
        ...
        if (Random.Shared.NextDouble() < 0.15) // 15% chance per update (higher than chordata)
        {
            var learningSystem = new NeuralLearningSystem(NeuralNetwork, Genome);
            learningSystem.Learn(0.08);
        }
    ...
    private void UpdateMovementParameters()
    {
        ...
        double activityFactor = NeuralNetwork.ActivityLevel * 0.4 + 0.6;
        MovementParameters.BaseSpeed = Math.Max(0.1, Math.Min(3.0, 
            MovementParameters.BaseSpeed * activityFactor));
        
        // Adjust gait complexity based on body flexibility
        int flexibilityGenes = Genome.CalculateEpistaticInteractions().Count(kvp => kvp.Key.Contains("flex") || kvp.Key.Contains("soft"));
    ...
    private void UpdateMeshParameters()
    {
        ...
        MeshParameters.BaseScale = Math.Max(0.3, Math.Min(1.5, 
            0.8 + (averageExpression - 0.5) * 1.4));
        
        // Update vertex count based on body complexity
        int bodyComplexityGenes = Genome.CalculateEpistaticInteractions().Count(kvp => kvp.Key.Contains("body") || kvp.Key.Contains("shape"));
        MeshParameters.VertexCount = Math.Max(800, 
            MeshParameters.VertexCount + bodyComplexityGenes * 150);
    }
